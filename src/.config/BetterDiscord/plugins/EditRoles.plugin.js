        ...
        return class EditRoles extends Plugin {
          ...
          onStart() {
            BDFDB.PatchUtils.patch(
              this,
              BDFDB.LibraryModules.GuildStore,
              "getGuild",
              {
                after: (e) => {
                  if (e.returnValue)
                    e.returnValue = this.changeRolesInGuild(
                      e.returnValue,
                      true
                    );
                },
            ...
            BDFDB.PatchUtils.patch(
              this,
              BDFDB.LibraryModules.PermissionRoleUtils,
              "getHighestRole",
              {
                after: (e) => {
                  if (e.returnValue && changedRoles[e.returnValue.id]) {
                    ...
                    e.returnValue = Object.assign({}, e.returnValue, {
                      ...
                      color: data.color
                        ? BDFDB.ColorUtils.convert(data.color, "INT")
                        : e.returnValue.color,
                      colorString: data.color
                        ? BDFDB.ColorUtils.convert(data.color, "HEX")
                        : e.returnValue.colorString,
                    });
                  }
                },
              }
            ...
            BDFDB.PatchUtils.patch(
              this,
              BDFDB.LibraryModules.MemberStore,
              "getMember",
              {
                after: (e) => {
                  if (e.returnValue) {
                    ...
                    if (guild) {
                      ...
                      for (let id of e.returnValue.roles)
                        if (
                          guild.roles[id] &&
                          guild.roles[id].colorString &&
                          (!role || role.position < guild.roles[id].position)
                        )
                          role = guild.roles[id];
          ...
          onUserContextMenu(e) {
            ...
            if (
              ...
              BDFDB.ArrayUtils.is(children[index].props.children)
            )
              for (let child of children[index].props.children) {
                if (
                  child &&
                  child.props &&
                  ...
                  child.props.label = (...args) => {
                    ...
                    let onContextMenu =
                      typeof label.props.onContextMenu == "function"
                        ? label.props.onContextMenu
                        : (_) => {};
                    label.props.onContextMenu = (event) => {
                      BDFDB.LibraryModules.ContextMenuUtils.openContextMenu(
                        event,
                        function (e) {
                          return BDFDB.ReactUtils.createElement(
                            BDFDB.ModuleUtils.findByName(
                              "DeveloperContextMenu"
                            ),
                            Object.assign({}, e, { id: child.props.id })
                          );
                        }
                      );
                    };
          ...
          onGuildContextMenu(e) {
            if (e.instance.props.guild)
              e.instance.props.guild = this.changeRolesInGuild(
                            ...
                            children: [
                                ...
                                BDFDB.LibraryComponents.MenuItems.MenuItem,
                                {
                                  ...
                                  action: (_) => {
                                    this.openRoleSettingsModal(
                                      guild.roles[e.instance.props.id]
                                    );
                                  },
                                }
                              ),
                              BDFDB.ContextMenuUtils.createItem(
                                BDFDB.LibraryComponents.MenuItems.MenuItem,
                                {
                                  ...
                                  color:
                                    BDFDB.LibraryComponents.MenuItems.Colors
                                      .DANGER,
                                  disabled: !changedRoles[e.instance.props.id],
                                  action: (event) => {
                                    let remove = (_) => {
                                      BDFDB.DataUtils.remove(
                                        this,
                                        "roles",
                                        e.instance.props.id
                                      );
          ...
          processChannelMembers(e) {
            ...
            for (let i in e.instance.props.groups)
              if (e.instance.props.groups[i].type == "GROUP") {
                let data = changedRoles[e.instance.props.groups[i].id];
                if (data && data.name)
                  e.instance.props.groups[i] = Object.assign(
                    {},
                    e.instance.props.groups[i],
                    { title: data.name }
                  );
              }
            e.instance.props.rows = [].concat(e.instance.props.rows);
            for (let i in e.instance.props.rows)
              if (e.instance.props.rows[i].type == "GROUP") {
                let data = changedRoles[e.instance.props.rows[i].id];
                if (data && data.name)
                  e.instance.props.rows[i] = Object.assign(
                    {},
                    e.instance.props.rows[i],
                    { title: data.name }
                  );
              }
          }
          ...
          processMemberListItem(e) {
            if (e.instance.props.user) {
              let member = BDFDB.LibraryModules.MemberStore.getMember(
          ...
          getGuildFromRoleId(roleId) {
            return BDFDB.LibraryModules.FolderStore.getFlattenedGuilds().find(
              (g) => g.roles[roleId]
            );
          }
          ...
          changeRolesInGuild(guild, useNative) {
            let changed = false,
              roles = Object.assign({}, guild.roles);
            for (let id in guild.roles) {
              ...
              if (data) {
                ...
                roles[id] = Object.assign({}, roles[id], {
                  ...
                  colorString: data.color
                    ? BDFDB.ColorUtils.convert(data.color, "HEX")
                    : roles[id].colorString,
                });
              }
            }
            if (useNative) guild.roles = roles;
            return !changed || useNative
              ? guild
              : new BDFDB.DiscordObjects.Guild(
                  Object.assign({}, guild, { roles })
                );
          }
            ...
            BDFDB.ModalUtils.open(this, {
              ...
              children: [
                BDFDB.ReactUtils.createElement(
                  ...
                  {
                    title:
                      BDFDB.LanguageUtils.LanguageStrings.FORM_LABEL_ROLE_NAME,
                ...
                BDFDB.ReactUtils.createElement(
                  ...
                  {
                    title:
                      BDFDB.LanguageUtils.LanguageStrings.FORM_LABEL_ROLE_COLOR,
                    className: BDFDB.disCN.marginbottom20,
                    children: [
                      BDFDB.ReactUtils.createElement(
                  ...
                  onClick: (_) => {
                    ...
                    if (
                      Object.keys(newData).every(
                        (key) => newData[key] == null || newData[key] == false
                      ) &&
                      (changed = true)
                    ) {
                      BDFDB.DataUtils.remove(this, "roles", role.id);
                    } else if (
                      !BDFDB.equals(newData, data) &&
                      (changed = true)
                    ) {
            ...
            switch (BDFDB.LanguageUtils.getLanguage().id) {
              case "bg": // Bulgarian
                return {
                  ...
                  confirm_resetall:
                    "Наистина ли искате да нулирате всички роли?",
              ...
              case "da": // Danish
                return {
                  confirm_reset:
                    "Er du sikker på, at du vil nulstille denne rolle?",
                  confirm_resetall:
                    "Er du sikker på, at du vil nulstille alle roller?",
              ...
              case "de": // German
                return {
                  confirm_reset:
                    "Möchtest du diese Rolle wirklich zurücksetzen?",
                  confirm_resetall:
                    "Möchtest du wirklich alle Rollen zurücksetzen?",
              ...
              case "el": // Greek
                return {
                  confirm_reset:
                    "Είστε βέβαιοι ότι θέλετε να επαναφέρετε αυτόν τον ρόλο;",
                  confirm_resetall:
                    "Είστε βέβαιοι ότι θέλετε να επαναφέρετε όλους τους ρόλους;",
              ...
              case "es": // Spanish
                return {
                  confirm_reset:
                    "¿Está seguro de que desea restablecer este rol?",
                  confirm_resetall:
                    "¿Está seguro de que desea restablecer todos los roles?",
              ...
              case "fr": // French
                return {
                  ...
                  confirm_resetall:
                    "Voulez-vous vraiment réinitialiser tous les rôles?",
              ...
              case "hr": // Croatian
                return {
                  confirm_reset:
                    "Jeste li sigurni da želite resetirati ovu ulogu?",
                  confirm_resetall:
                    "Jeste li sigurni da želite resetirati sve uloge?",
              ...
              case "hu": // Hungarian
                return {
                  confirm_reset:
                    "Biztosan vissza akarja állítani ezt a szerepet?",
                  confirm_resetall:
                    "Biztosan vissza akarja állítani az összes szerepet?",
              ...
              case "it": // Italian
                return {
                  confirm_reset:
                    "Sei sicuro di voler reimpostare questo ruolo?",
                  confirm_resetall:
                    "Sei sicuro di voler reimpostare tutti i ruoli?",
              ...
              case "lt": // Lithuanian
                return {
                  confirm_reset:
                    "Ar tikrai norite iš naujo nustatyti šį vaidmenį?",
                  confirm_resetall:
                    "Ar tikrai norite iš naujo nustatyti visus vaidmenis?",
              ...
              case "nl": // Dutch
                return {
                  ...
                  confirm_resetall:
                    "Weet u zeker dat u alle rollen opnieuw wilt instellen?",
              ...
              case "no": // Norwegian
                return {
                  confirm_reset:
                    "Er du sikker på at du vil tilbakestille denne rollen?",
                  confirm_resetall:
                    "Er du sikker på at du vil tilbakestille alle rollene?",
              ...
              case "pl": // Polish
                return {
                  ...
                  confirm_resetall:
                    "Czy na pewno chcesz zresetować wszystkie role?",
              ...
              case "pt-BR": // Portuguese (Brazil)
                return {
                  confirm_reset:
                    "Tem certeza de que deseja redefinir esta função?",
                  confirm_resetall:
                    "Tem certeza de que deseja redefinir todas as funções?",
              ...
              case "sv": // Swedish
                return {
                  confirm_reset:
                    "Är du säker på att du vill återställa denna roll?",
                  confirm_resetall:
                    "Är du säker på att du vill återställa alla roller?",
              ...
              case "tr": // Turkish
                return {
                  confirm_reset:
                    "Bu Rolü sıfırlamak istediğinizden emin misiniz?",
                  confirm_resetall:
                    "Tüm Rolleri sıfırlamak istediğinizden emin misiniz?",
              ...
              case "vi": // Vietnamese
                return {
                  confirm_reset:
                    "Bạn có chắc chắn muốn đặt lại Vai trò này không?",
                  confirm_resetall:
                    "Bạn có chắc chắn muốn đặt lại tất cả các Vai trò không?",
