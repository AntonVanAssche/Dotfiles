...
var MemberCount = (() => {
  ...
  const buildPlugin = ([Plugin, Api]) => {
    ...
    const Menu = WebpackModules.getByProps(
      "MenuItem",
      "MenuGroup",
      "MenuSeparator"
    );
    const GuildContextMenu = WebpackModules.find(
      (mod) =>
        mod && mod.default && mod.default.displayName === "GuildContextMenu"
    );
            ...
            React.createElement("h2", {
              ...
              children: [
                React.createElement("span", {
                  children: [Counter.strings.MEMBERS, "—", this.props.count],
                }),
              ],
            }),
          ],
    ...
    return class MemberCount extends Plugin {
      ...
      patchMemberList(state) {
        ...
        Patcher.after(Lists.ListThin, "render", (that, args, value) => {
          ...
          if (!props || !props.id || !props.id.startsWith("members"))
            return value;
          ...
          if (this.settings.blacklist.includes(guildId) || !guildId) {
            if (list && list.classList.contains("hasCounter"))
              list.classList.remove("hasCounter");
            return value;
          }
          ...
          const fn = (item) =>
            item && item.key && item.key.startsWith("MemberCount");

          if (!Array.isArray(value)) value = [value];
          if (!value.some(fn)) {
            value.unshift(counter);
            if (list && !list.classList.contains("hasCounter"))
              list.classList.add("hasCounter");
          }
      ...
      updateMemberList(scroll) {
        const memberList = document.querySelector(
          DiscordSelectors.MemberList.members.value.trim()
        );
      ...
      patchGuildContextMenu(state) {
        ...
        const fn = (item) =>
          item && item.key && item.key === "MemberCount-Group";
      ...
      updateContextPosition(that) {
        if (!that) return;
        const height =
          this.getProps(that, "updatePosition") ||
          this.getProps(that, "props.onHeightUpdate") ||
          this.getProps(
            that,
            "_reactInternalFiber.return.memoizedProps.onHeightUpdate"
          ) ||
          this.getProps(
            that,
            "_reactInternalFiber.child.child.memoizedProps.onHeightUpdate"
          );
      ...
      updateContextMenus() {
        ...
        for (const menu of menus)
          ReactTools.getOwnerInstance(menu).forceUpdate();
      }
      ...
      processContextMenu(cm) {
        ...
        if (
          !readItem ||
          Array.isArray(readItem) ||
          readItem.props.id === "mark-folder-read"
        )
          return;
        if (readItem.props.id === "mark-guild-read")
          return this.addGuildContextItems(inst, own, cm);
      }

      addGuildContextItems(instance, owner, context) {
        const group = new ContextMenu.ItemGroup();
        const ref = owner.props.children(
          { position: owner.props.reference() },
        ...
        group.addItems(item);
        context.firstChild.firstChild.firstChild.insertAdjacentElement(
          "afterend",
          groupEl
        );
      ...
      loadSettings() {
        ...
        if (Array.isArray(data))
          return (this.settings = { blacklist: [...data] });
      ...
      getProps(obj, path) {
        return path
          .split(/\s?\.\s?/)
          .reduce((object, prop) => object && object[prop], obj);
      }
