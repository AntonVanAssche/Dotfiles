...
module.exports = ((_) => {
  const config = {
    ...
    changeLog: {
      improved: {
        Threads:
          "Added the Channel Info Modal for Threads, it's not possible to see a list of all threads in a server, because Discord only sends you the info a thread after you attempted to join it, which isn't possible with private threads (it's similar like server invites)",
      },
    },
  };

  return window.Lightcord || window.LightCord
    ? class {
        getName() {
    ...
    : (([Plugin, BDFDB]) => {
        ...
        const sortOrders = {
          NATIVE: {
            value: "native",
            label: "Native Category in correct Order",
          },
        ...
        const UserRowComponent = class UserRow extends BdApi.React.Component {
          componentDidMount() {
            if (this.props.user.fetchable) {
              ...
              BDFDB.LibraryModules.UserFetchUtils.getUser(
                this.props.user.id
              ).then((fetchedUser) => {
                this.props.user = Object.assign(
                  {},
                  fetchedUser,
                  BDFDB.LibraryModules.MemberStore.getMember(
                    this.props.guildId,
                    this.props.user.id
                  ) || {}
                );
                ...
                label: [
                  ...
                  this.props.user.bot &&
                    BDFDB.ReactUtils.createElement(
                      BDFDB.LibraryComponents.BotTag,
                      {
                        style: { marginLeft: 6 },
                      }
                    ),
          ...
          render() {
            return BDFDB.ReactUtils.createElement(
              BDFDB.LibraryComponents.ListRow,
              {
                prefix: BDFDB.ReactUtils.createElement("div", {
                  className:
                    BDFDB.disCNS.avataricon +
                    BDFDB.disCNS.listavatar +
                    BDFDB.disCNS.avatariconsizemedium +
                    BDFDB.disCN.avatariconinactive,
                  style: {
                    ...
                    style: {
                      ...
                      backgroundColor:
                        BDFDB.ColorUtils.convert(
                          this.props.role.colorString,
                          "RGB"
                        ) || BDFDB.DiscordConstants.Colors.PRIMARY_DARK_300,
                    },
                  }),
                }),
                labelClassName:
                  this.props.role.overwritten && BDFDB.disCN.strikethrough,
        ...
        return class ShowHiddenChannels extends Plugin {
          onLoad() {
            overrideTypes = Object.keys(
              BDFDB.DiscordConstants.PermissionOverrideType
            );
            ...
            this.defaults = {
              ...
              general: {
                alwaysCollapse: {
                  value: false,
                  description:
                    "Always collapse 'Hidden' Category after switching Servers",
                },
                showVoiceUsers: {
                  value: true,
                  description: "Show connected Users in hidden Voice Channels",
                },
                showForNormal: {
                  value: true,
                  description:
                    "Add Access-Overview ContextMenu Entry for non-hidden Channels",
                },
              },
              channels: {
            ...
            this.css = `
					${
            BDFDB.dotCNS._showhiddenchannelsaccessmodal +
            BDFDB.dotCN.messagespopoutemptyplaceholder
          } {
						position: absolute;
						bottom: 0;
						width: 100%;
          ...
          onStart() {
            ...
            this.saveCollapseList(
              !BDFDB.ArrayUtils.is(loadedCollapseList) ? [] : loadedCollapseList
            );

            BDFDB.PatchUtils.patch(
              this,
              BDFDB.LibraryModules.UnreadChannelUtils,
              "hasUnread",
              {
                after: (e) => {
                  return (
                    e.returnValue && !this.isChannelHidden(e.methodArguments[0])
                  );
                },
              }
            );
            ...
            BDFDB.PatchUtils.patch(
              ...
              {
                after: (e) => {
                  return this.isChannelHidden(e.methodArguments[0])
                    ? 0
                    : e.returnValue;
                },
              }
            ...
            BDFDB.PatchUtils.patch(
              this,
              BDFDB.LibraryModules.CategoryCollapseStore,
              "isCollapsed",
              {
                after: (e) => {
                  if (
                    e.methodArguments[0] &&
                    e.methodArguments[0].endsWith("hidden")
                  ) {
                    if (
                      this.settings.general.alwaysCollapse &&
                      e.methodArguments[0] != lastGuildId &&
                      !collapseList.includes(e.methodArguments[0])
                    ) {
                      collapseList.push(e.methodArguments[0]);
                      this.saveCollapseList(
                        BDFDB.ArrayUtils.removeCopies(collapseList)
                      );
                    }
            ...
            BDFDB.PatchUtils.patch(
              this,
              BDFDB.LibraryModules.CategoryCollapseUtils,
              "categoryCollapse",
              {
                before: (e) => {
                  if (
            ...
            BDFDB.PatchUtils.patch(
              this,
              BDFDB.LibraryModules.GuildChannelStore,
              "getTextChannelNameDisambiguations",
              {
                after: (e) => {
                  ...
                  for (let channel_id in all)
                    if (
                      all[channel_id].guild_id == e.methodArguments[0] &&
                      !e.returnValue[channel_id] &&
                      all[channel_id].type !=
                        BDFDB.DiscordConstants.ChannelTypes.GUILD_CATEGORY &&
                      all[channel_id].type !=
                        BDFDB.DiscordConstants.ChannelTypes.GUILD_VOICE
                    )
                      e.returnValue[channel_id] = {
                        id: channel_id,
                        name: all[channel_id].name,
                      };
                },
              }
            );
            ...
            BDFDB.PatchUtils.patch(
              this,
              BDFDB.LibraryModules.ChannelIconUtils,
              "getChannelIconComponent",
              {
                before: (e) => {
                  if (
                    e.methodArguments[2] &&
                    e.methodArguments[2].locked &&
                    e.methodArguments[0] &&
                    this.isChannelHidden(e.methodArguments[0].id)
                  )
                    e.methodArguments[2].locked = false;
                },
              }
            );
                  ...
                  settingsItems.push(
                    BDFDB.ReactUtils.createElement(
                      BDFDB.LibraryComponents.CollapseContainer,
                      {
                        title: "Settings",
                        collapseStates: collapseStates,
                        children: Object.keys(this.defaults.sortOrder)
                          .map((key) =>
                            BDFDB.ReactUtils.createElement(
                          ...
                          .concat(
                            Object.keys(this.defaults.general).map((key) =>
                              BDFDB.ReactUtils.createElement(
                                BDFDB.LibraryComponents.SettingsSaveItem,
                                {
                                  type: "Switch",
                        ...
                        children: [
                          ...
                          BDFDB.ReactUtils.createElement(
                            BDFDB.LibraryComponents.SettingsItem,
                            {
                              type: "Button",
                              color:
                                BDFDB.LibraryComponents.Button.Colors.GREEN,
                              ...
                              children:
                                BDFDB.LanguageUtils.LanguageStrings.ENABLE,
                            }
                          ),
                          BDFDB.ReactUtils.createElement(
                            BDFDB.LibraryComponents.SettingsItem,
                            {
                              type: "Button",
                              color:
                                BDFDB.LibraryComponents.Button.Colors.PRIMARY,
                              ...
                              children:
                                BDFDB.LanguageUtils.LanguageStrings.DISABLE,
                            }
                          ),
                        ],
                      }
          ...
          onChannelContextMenu(e) {
            if (e.instance.props.channel) {
              if (
                e.instance.props.channel.id.endsWith("hidden") &&
                e.instance.props.channel.type ==
                  BDFDB.DiscordConstants.ChannelTypes.GUILD_CATEGORY
              ) {
                let [children, index] = BDFDB.ReactUtils.findParent(
                  e.returnvalue,
                  { name: "ChannelMuteItem" }
                );
                if (index > -1) children.splice(index, 1);
                [children, index] = BDFDB.ContextMenuUtils.findItem(
                  e.returnvalue,
                  { id: "mark-channel-read", group: true }
                );
                children.splice(
                  index > -1 ? index + 1 : 0,
                  0,
                    ...
                    BDFDB.LibraryComponents.MenuItems.MenuGroup,
                    {
                      children: BDFDB.ContextMenuUtils.createItem(
                        BDFDB.LibraryComponents.MenuItems.MenuItem,
                        {
                          ...
                          children: Object.keys(sortOrders)
                            .filter(
                              (n) =>
                                sortOrders[n].value != sortOrders.EXTRA.value
                            )
                            .map((n) =>
                              BDFDB.ContextMenuUtils.createItem(
                                BDFDB.LibraryComponents.MenuItems.MenuGroup,
                                {
                                  children: BDFDB.ContextMenuUtils.createItem(
                                    BDFDB.LibraryComponents.MenuItems.MenuItem,
                                    {
                                      ...
                                      action: (_) => {
                                        this.settings.sortOrder.hidden =
                                          sortOrders[n].value;
          ...
          processChannels(e) {
            if (
              ...
              blackList.includes(e.instance.props.guild.id)
            )
              return;
            let [hiddenChannels, amount] = this.getHiddenChannels(
              e.instance.props.guild
            );
            if (amount) {
              e.instance.props.categories = Object.assign(
                {},
                e.instance.props.categories
              );
              for (let catId in e.instance.props.categories)
                e.instance.props.categories[catId] = [].concat(
                  e.instance.props.categories[catId]
                );
              e.instance.props.channels = Object.assign(
                {},
                e.instance.props.channels
              );
              for (let type in e.instance.props.channels)
                e.instance.props.channels[type] = [].concat(
                  e.instance.props.channels[type]
                );

              let hiddenId = e.instance.props.guild.id + "_hidden";

              delete e.instance.props.categories[hiddenId];
              e.instance.props.categories._categories =
                e.instance.props.categories._categories.filter(
                  (n) => n.channel.id != hiddenId
                );
              e.instance.props.channels[
                BDFDB.DiscordConstants.ChannelTypes.GUILD_CATEGORY
              ] = e.instance.props.channels[
                BDFDB.DiscordConstants.ChannelTypes.GUILD_CATEGORY
              ].filter((n) => n.channel.id != hiddenId);

              let index = -1;
              for (let catId in e.instance.props.categories) {
                if (catId != "_categories")
                  e.instance.props.categories[catId] =
                    e.instance.props.categories[catId].filter(
                      (n) => !this.isChannelHidden(n.channel.id)
                    );
                for (let channelObj of e.instance.props.categories[catId])
              ...
              for (let type in hiddenChannels) {
                let channelType =
                  channelGroupMap[BDFDB.DiscordConstants.ChannelTypes[type]] ||
                  type;
                if (
                  !BDFDB.ArrayUtils.is(e.instance.props.channels[channelType])
                )
                  e.instance.props.channels[channelType] = [];
                for (let channel of hiddenChannels[type]) {
                  let hiddenChannel = new BDFDB.DiscordObjects.Channel(
                    Object.assign({}, channel, {
                      parent_id: hiddenCategory ? hiddenId : channel.parent_id,
                      position:
                        this.settings.sortOrder.hidden ==
                        sortOrders.BOTTOM.value
                          ? 999999999
                          : channel.position,
                    })
                  );
              ...
              for (let parent_id in e.instance.props.categories)
                BDFDB.ArrayUtils.keySort(
                  e.instance.props.categories[parent_id],
                  "index"
                );
              for (let channelType in e.instance.props.channels)
                BDFDB.ArrayUtils.keySort(
                  e.instance.props.channels[channelType],
                  "comparator"
                );
          ...
          processChannelCategoryItem(e) {
            if (
              ...
              e.instance.props.channel = hiddenCategory;
          }

          processChannelItem(e) {
            if (
              e.instance.props.channel &&
              this.isChannelHidden(e.instance.props.channel.id)
            ) {
              if (!e.returnvalue)
                e.instance.props.className = BDFDB.DOMUtils.formatClassName(
                  e.instance.props.className,
                  BDFDB.disCN._showhiddenchannelshiddenchannel
                );
              else {
                let [children, index] = BDFDB.ReactUtils.findParent(
                  e.returnvalue,
                  { name: "ChannelItemIcon" }
                );
                let channelChildren = BDFDB.ReactUtils.findChild(
                  e.returnvalue,
                  { props: [["className", BDFDB.disCN.channelchildren]] }
                );
                if (
                  channelChildren &&
                  channelChildren.props &&
                  channelChildren.props.children
                ) {
                  channelChildren.props.children = [
                    BDFDB.ReactUtils.createElement(
                      BDFDB.LibraryComponents.TooltipContainer,
                      {
                        text: BDFDB.LanguageUtils.LanguageStrings
                          .CHANNEL_LOCKED_SHORT,
                        children: BDFDB.ReactUtils.createElement(
                ...
                if (
                  !(
                    e.instance.props.channel.type ==
                      BDFDB.DiscordConstants.ChannelTypes.GUILD_VOICE &&
                    e.instance.props.connected
                  )
                ) {
                  let wrapper = BDFDB.ReactUtils.findChild(e.returnvalue, {
                    props: [["className", BDFDB.disCN.channelwrapper]],
                  });
                  if (wrapper) {
                    wrapper.props.onMouseDown = (_) => {};
                    wrapper.props.onMouseUp = (_) => {};
                  }
                  let mainContent = BDFDB.ReactUtils.findChild(e.returnvalue, {
                    props: [["className", BDFDB.disCN.channelmaincontent]],
                  });
                  if (mainContent) {
          ...
          processVoiceUsers(e) {
            if (
              !this.settings.general.showVoiceUsers &&
              this.isChannelHidden(e.instance.props.channel.id)
            )
          ...
          isChannelHidden(channelId) {
            let channel =
              BDFDB.LibraryModules.ChannelStore.getChannel(channelId);
            return (
              channel &&
              hiddenChannelCache[channel.guild_id] &&
              hiddenChannelCache[channel.guild_id].hidden[channel.type] &&
              hiddenChannelCache[channel.guild_id].hidden[channel.type].find(
                (c) => c.id == channel.id
              )
            );
          }
          ...
          getAllChannels() {
            return (
              BDFDB.LibraryModules.ChannelStore.getGuildChannels ||
              BDFDB.LibraryModules.ChannelStore.getMutableGuildChannels ||
              ((_) => ({}))
            )();
          }
          ...
          getHiddenChannels(guild) {
            ...
            let hiddenChannels = {},
              visibleAmount =
                BDFDB.LibraryModules.GuildChannelStore.getChannels(
                  guild.id
                ) || { count: 0 },
              count,
              rolesAmount = (
            ...
            if (
              !hiddenChannelCache[guild.id] ||
              hiddenChannelCache[guild.id].visible != visibleAmount ||
              hiddenChannelCache[guild.id].roles != rolesAmount
            ) {
              let all = this.getAllChannels();
              for (let type in BDFDB.DiscordConstants.ChannelTypes)
                hiddenChannels[BDFDB.DiscordConstants.ChannelTypes[type]] = [];
              for (let channel_id in all) {
                ...
                if (
                  ...
                  (this.settings.channels[
                    BDFDB.DiscordConstants.ChannelTypes[channel.type]
                  ] ||
                    this.settings.channels[
                      BDFDB.DiscordConstants.ChannelTypes[channel.type]
                    ] === undefined) &&
                  !BDFDB.DMUtils.isDMChannel(channel.id) &&
                  !BDFDB.UserUtils.can(
                    "VIEW_CHANNEL",
                    BDFDB.UserUtils.me.id,
                    channel.id
            ...
            for (let type in hiddenChannels)
              hiddenChannels[type] = hiddenChannels[type].filter((c) =>
                BDFDB.LibraryModules.ChannelStore.getChannel(c.id)
              );
            hiddenChannelCache[guild.id] = {
              hidden: hiddenChannels,
              amount: BDFDB.ObjectUtils.toArray(hiddenChannels).flat().length,
              visible: visibleAmount,
              roles: rolesAmount,
            };
            return [
              hiddenChannelCache[guild.id].hidden,
              hiddenChannelCache[guild.id].amount,
            ];
          }
          ...
          openAccessModal(channel, allowed) {
            ...
            let guild = BDFDB.LibraryModules.GuildStore.getGuild(
              channel.guild_id
            );
            let myMember =
              guild &&
            ...
            let parentChannel =
              ...
              BDFDB.LibraryModules.ChannelStore.getChannel(
                BDFDB.LibraryModules.ChannelStore.getChannel(channel.id)
                  .parent_id
              );
            let category =
              (parentChannel &&
              ...
              if (user)
                users.push(
                  Object.assign(
                    {},
                    user,
                    BDFDB.LibraryModules.MemberStore.getMember(guild.id, id) ||
                      {}
                  )
                );
              else
            ...
            let checkPerm = (permString) => {
              return (
                (permString |
                  BDFDB.DiscordConstants.Permissions.VIEW_CHANNEL) ==
                  permString ||
                (permString |
                  BDFDB.DiscordConstants.Permissions.READ_MESSAGE_HISTORY) ==
                  permString ||
                (channel.type ==
                  BDFDB.DiscordConstants.ChannelTypes.GUILD_VOICE &&
                  (permString | BDFDB.DiscordConstants.Permissions.CONNECT) ==
                    permString)
              );
            };
            ...
            for (let id in channel.permissionOverwrites) {
              if (
                (channel.permissionOverwrites[id].type ==
                  BDFDB.DiscordConstants.PermissionOverrideType.ROLE ||
                  overrideTypes[channel.permissionOverwrites[id].type] ==
                    BDFDB.DiscordConstants.PermissionOverrideType.ROLE) &&
                guild.roles[id] &&
                guild.roles[id].name != "@everyone" &&
                checkPerm(channel.permissionOverwrites[id].allow)
              ) {
                allowedRoles.push(
                  Object.assign(
                    {
                      overwritten:
                        myMember && myMember.roles.includes(id) && !allowed,
                    },
                    guild.roles[id]
                  )
                );
                ...
                (channel.permissionOverwrites[id].type ==
                  BDFDB.DiscordConstants.PermissionOverrideType.MEMBER ||
                  overrideTypes[channel.permissionOverwrites[id].type] ==
                    BDFDB.DiscordConstants.PermissionOverrideType.MEMBER) &&
                checkPerm(channel.permissionOverwrites[id].allow)
              ) {
              ...
              if (
                (channel.permissionOverwrites[id].type ==
                  ...
                  overrideTypes[channel.permissionOverwrites[id].type] ==
                    BDFDB.DiscordConstants.PermissionOverrideType.ROLE) &&
                checkPerm(channel.permissionOverwrites[id].deny)
              ) {
                deniedRoles.push(guild.roles[id]);
                if (guild.roles[id] && guild.roles[id].name == "@everyone")
                  everyoneDenied = true;
              } else if (
                (channel.permissionOverwrites[id].type ==
                  BDFDB.DiscordConstants.PermissionOverrideType.MEMBER ||
            ...
            if (
              ![]
                .concat(allowedUsers, deniedUsers)
                .find((user) => user.id == guild.ownerId)
            )
              addUser(guild.ownerId, allowedUsers);
            for (let id in guild.roles)
              if (
                (guild.roles[id].permissions |
                  BDFDB.DiscordConstants.Permissions.ADMINISTRATOR) ==
                  guild.roles[id].permissions &&
                ![]
                  .concat(allowedRoles, deniedRoles)
            ...
            for (let role of allowedRoles)
              allowedElements.push(
                BDFDB.ReactUtils.createElement(RoleRowComponent, {
                  role: role,
                  guildId: guild.id,
            ...
            for (let user of allowedUsers)
              allowedElements.push(
                BDFDB.ReactUtils.createElement(UserRowComponent, {
                  user: user,
                  guildId: guild.id,
                  channelId: channel.id,
                })
            ...
            const infoStrings = [
              ...
              !isThread && {
                title:
                  BDFDB.LanguageUtils.LanguageStrings.FORM_LABEL_CHANNEL_NAME,
                text: channel.name,
              },
              channel.type == BDFDB.DiscordConstants.ChannelTypes.GUILD_VOICE
                ? {
                    title:
                      BDFDB.LanguageUtils.LanguageStrings.FORM_LABEL_BITRATE,
                ...
                : {
                    ...
                    text: BDFDB.ReactUtils.markdownParse(
                      channel.topic || "---"
                    ),
                  },
              {
                ...
                text: BDFDB.LanguageUtils.LanguageStrings[
                  typeNameMap[BDFDB.DiscordConstants.ChannelTypes[channel.type]]
                ],
              },
              isThread &&
                parentChannel && {
                ...
                },
              {
                ...
                text:
                  ...
                  BDFDB.LanguageUtils.LanguageStrings.NO_CATEGORY,
              },
            ]
              .map(
                (formLabel, i) =>
                    ...
                    BDFDB.ReactUtils.createElement(
                      BDFDB.LibraryComponents.FormComponents.FormItem,
                      {
                        ...
                        className: BDFDB.DOMUtils.formatClassName(
                          ...
                          i == 0 && BDFDB.disCN.margintop8
                        ),
                        children: BDFDB.ReactUtils.createElement(
                          BDFDB.LibraryComponents.FormComponents.FormText,
                          {
            ...
            BDFDB.ModalUtils.open(this, {
              size: "MEDIUM",
              header:
                BDFDB.LanguageUtils.LanguageStrings.CHANNEL +
                " " +
                BDFDB.LanguageUtils.LanguageStrings.ACCESSIBILITY,
              subHeader: "#" + channel.name,
              className: BDFDB.disCN._showhiddenchannelsaccessmodal,
              contentClassName: BDFDB.DOMUtils.formatClassName(
                !isThread && BDFDB.disCN.listscroller
              ),
              onOpen: (modalInstance) => {
                if (modalInstance) accessModal = modalInstance;
              },
              children: isThread
                ? infoStrings
                : [
                    BDFDB.ReactUtils.createElement(
                      BDFDB.LibraryComponents.ModalComponents.ModalTabContent,
                      {
                        ...
                        tab: BDFDB.LanguageUtils.LanguageStrings
                          .OVERLAY_SETTINGS_GENERAL_TAB,
                    ...
                    BDFDB.ReactUtils.createElement(
                      BDFDB.LibraryComponents.ModalComponents.ModalTabContent,
                      {
                        ...
                        children: allowedElements.length
                          ? allowedElements
                          : BDFDB.ReactUtils.createElement(
                              BDFDB.LibraryComponents.MessagesPopoutComponents
                                .EmptyStateBottom,
                              {
                                msg: BDFDB.LanguageUtils.LanguageStrings
                    ...
                    BDFDB.ReactUtils.createElement(
                      BDFDB.LibraryComponents.ModalComponents.ModalTabContent,
                      {
                        ...
                        children: deniedElements.length
                          ? deniedElements
                          : BDFDB.ReactUtils.createElement(
                              BDFDB.LibraryComponents.MessagesPopoutComponents
                                .EmptyStateBottom,
            ...
            switch (BDFDB.LanguageUtils.getLanguage().id) {
              ...
              case "cs": // Czech
                return {
                  ...
                  context_changeorder_native:
                    "Nativní kategorie ve správném pořadí",
              ...
              case "da": // Danish
                return {
                  ...
                  context_changeorder_native:
                    "Native Kategori i korrekt rækkefølge",
              ...
              case "de": // German
                return {
                  context_changeorder:
                    "Reihenfolge der versteckten Kanäle ändern",
                  context_changeorder_bottom: "Native Kategorie ganz unten",
                  context_changeorder_native:
                    "Native Kategorie in der richtigen Reihenfolge",
              ...
              case "el": // Greek
                return {
                  ...
                  context_changeorder_bottom:
                    "Εγγενής κατηγορία στο κάτω μέρος",
                  context_changeorder_native:
                    "Εγγενής κατηγορία σε σωστή σειρά",
              ...
              case "es": // Spanish
                return {
                  context_changeorder:
                    "Cambiar el orden de los canales ocultos",
                  context_changeorder_bottom:
                    "Categoría nativa en la parte inferior",
                  context_changeorder_native:
                    "Categoría nativa en el orden correcto",
              ...
              case "fi": // Finnish
                return {
                  context_changeorder:
                    "Muuta piilotettujen kanavien järjestystä",
                  context_changeorder_bottom: "Alkuperäinen luokka alareunassa",
                  context_changeorder_native:
                    "Alkuperäinen luokka oikeassa järjestyksessä",
              ...
              case "fr": // French
                return {
                  ...
                  context_changeorder_native:
                    "Catégorie native dans le bon ordre",
              ...
              case "hr": // Croatian
                return {
                  context_changeorder:
                    "Promijenite redoslijed skrivenih kanala",
                  context_changeorder_bottom: "Izvorna kategorija na dnu",
                  context_changeorder_native:
                    "Izvorna kategorija u ispravnom redoslijedu",
              ...
              case "hu": // Hungarian
                return {
                  context_changeorder:
                    "Rejtett csatornák sorrendjének módosítása",
                  context_changeorder_bottom: "Natív kategória az alján",
                  context_changeorder_native:
                    "Natív kategória helyes sorrendben",
              ...
              case "it": // Italian
                return {
                  ...
                  context_changeorder_native:
                    "Categoria nativa nell'ordine corretto",
              ...
              case "lt": // Lithuanian
                return {
                  ...
                  context_changeorder_native:
                    "Gimtoji kategorija teisinga tvarka",
              ...
              case "nl": // Dutch
                return {
                  context_changeorder:
                    "Wijzig de volgorde van verborgen kanalen",
                  context_changeorder_bottom: "Native categorie onderaan",
                  context_changeorder_native:
                    "Native categorie in de juiste volgorde",
              ...
              case "no": // Norwegian
                return {
                  ...
                  context_changeorder_native:
                    "Innfødt kategori i riktig rekkefølge",
                  context_channelaccess: "Kanaltilgang",
                  context_hidehidden: "Skjul låste kanaler",
              ...
              case "pl": // Polish
                return {
                  ...
                  context_changeorder_native:
                    "Kategoria natywna we właściwej kolejności",
              ...
              case "pt-BR": // Portuguese (Brazil)
                return {
                  ...
                  context_changeorder_bottom:
                    "Categoria nativa na parte inferior",
                  context_changeorder_native:
                    "Categoria nativa na ordem correta",
              ...
              case "ro": // Romanian
                return {
                  ...
                  context_changeorder_bottom:
                    "Categorie nativă în partea de jos",
                  context_changeorder_native:
                    "Categorie nativă în ordine corectă",
              ...
              case "ru": // Russian
                return {
                  ...
                  context_changeorder_native:
                    "Собственная категория в правильном порядке",
              ...
              case "sv": // Swedish
                return {
                  ...
                  context_changeorder_native:
                    "Naturlig kategori i rätt ordning",
              ...
              case "th": // Thai
                return {
                  ...
                  context_changeorder_native:
                    "หมวดหมู่ดั้งเดิมในลำดับที่ถูกต้อง",
              ...
              case "uk": // Ukrainian
                return {
                  ...
                  context_changeorder_native:
                    "Рідна категорія в правильному порядку",
              ...
              default:
                ...
                return {
                  ...
                  context_changeorder_native:
                    "Native Category in correct Order",
