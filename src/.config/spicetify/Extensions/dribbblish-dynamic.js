...
new Spicetify.ContextMenu.Item(
  ...
  ([uri]) => {
    ...
    localStorage.setItem(
      "sidebar-app-hide-list",
      JSON.stringify(appHiddenList)
    );
    ...
    */
    ...
    function replaceTextWithIcon(el, iconName) {
      ...
      el.parentNode.setAttribute(
        "data-uri",
        "spotify:special:sidebarapp:" + href
      );
      el.innerText = "";
    }
...
let mainColor = getComputedStyle(document.documentElement).getPropertyValue(
  "--modspotify_main_fg"
);
let mainColor2 = getComputedStyle(document.documentElement).getPropertyValue(
  "--modspotify_main_bg"
);
...
function updateColors(root) {
  ...
  iconCol = getComputedStyle(document.documentElement).getPropertyValue(
    isLightFg ? "--modspotify_main_bg" : "--modspotify_secondary_fg"
  );
...
async function songchange() {
  ...
  document.documentElement.style.setProperty(
    "--image_url",
    'url("' + Spicetify.Player.data.track.metadata.image_url + '")'
  );
...
new Spicetify.Menu.Item("About", false, () =>
  window.open("spotify:app:about")
).register();
